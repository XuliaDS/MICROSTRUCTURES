...
static void getFieldConditions( UserMicroLocalDataStruct *TVfield,  CagdRType tMin[], CagdRType tMax[],  UserMicroTileBndryPrmStruct *params )
{
  ...
  for ( i = 0 ; i < 3; i++ )
    centre[i] = 0.5 * ( tMin[i] + tMax[i] ) ;
  for (i = 0 ; i < 6; i++ ) {
      for ( j = 0 ; j < 4; ++j )
	params[i].Bndry[j] = 0.0;
      params[i].Circular   = CIRC;
  }
  f[0] = partialDerMag ( TVfield -> DuDefMap, tMin  [0], centre[1], centre[2] );
  f[1] = partialDerMag ( TVfield -> DuDefMap, tMax  [0], centre[1], centre[2] );
  f[2] = partialDerMag ( TVfield -> DvDefMap, centre[0], tMin  [1], centre[2] );
  f[3] = partialDerMag ( TVfield -> DvDefMap, centre[0], tMax  [1], centre[2] );
  f[4] = partialDerMag ( TVfield -> DwDefMap, centre[0], centre[1], tMin  [2] );
  f[5] = partialDerMag ( TVfield -> DwDefMap, centre[0], centre[1], tMax  [2] );
  printf(" ------------- FIELD DERIVATIVES ------------\n");
  for ( i = 0 ; i < 3; i ++) {
      printf(" i = %d  ->  param =  %f   Df %f \n", 2*i    , tMin[i], f[2*i    ] );
      printf(" i = %d  ->  param =  %f   Df %f \n", 2*i + 1, tMax[i], f[2*i + 1] );
  }
  printf("---------------------------------------------\n");
  ...
  for ( i = 0 ; i < 2; i++ ) {
      ...
      if      ( w <= 0.0 || w >= 1.0 ) {
	  norm[0] = partialDerMag ( TVfield -> DwDefMap, tMin[0], tMin[1], w );
	  norm[1] = partialDerMag ( TVfield -> DwDefMap, tMax[0], tMin[1], w );
	  norm[2] = partialDerMag ( TVfield -> DwDefMap, tMin[0], tMax[1], w );
	  norm[3] = partialDerMag ( TVfield -> DwDefMap, tMax[0], tMax[1], w );
	  for ( j = 0 ; j < 4; ++j )
  ...
  for ( j = i = 0 ; i < 6; i++ ) {
      ...
      params[i].OuterRadius = 0.05 * ( 2*j + 1 ) * f[i]/TVfield ->fMax[i] ;
  }
...
{ 
  int nu, nv, nw;
  if ( argc < 6 ) {
      printf(" I NEED A DOMAIN FILE (*.itd) + number of tiles per dir\n");
      return -1;
  }
